...
import com.jiankangli.knowledge.jiankang_yixiupro.bean.upkeepBlBean.WorkTemplateVosBean.TemplateTypeAndTemplateVosBean;
 ...
 * @description :模板适配器
 */
public class upkeep_TemplateAdapter extends BaseQuickAdapter<upkeepBlBean.WorkTemplateVosBean, BaseViewHolder> {


    public upkeep_TemplateAdapter(@Nullable List<upkeepBlBean.WorkTemplateVosBean> data) {
        super(data);
        //Step.1
        setMultiTypeDelegate(new MultiTypeDelegate<upkeepBlBean.WorkTemplateVosBean>() {
            @Override
            protected int getItemType(upkeepBlBean.WorkTemplateVosBean entity) {
                //根据你的实体类来判断布局类型
    ...
    @Override
    protected void convert(BaseViewHolder helper, final upkeepBlBean.WorkTemplateVosBean item) {
        //得到当前操作项的位置
