...
import com.jiankangli.knowledge.jiankang_yixiupro.Fragment.bl_upkeep.bl_upkeep_3_fragment;
 ...
 * @description :巡检-录入工单第二个界面
 */
public class bl_polling_2_fragment extends BaseFragment {
    ...
    public static bl_polling_2_fragment newInstance() {
        bl_polling_2_fragment fragment = new bl_polling_2_fragment();
    ...
    private void initRecordList() {
        ...
        List<pollingBlBean.ServiceRecodeVosBean> serviceRecodeVos = blBean.getServiceRecodeVos();
        if (serviceRecodeVos == null) {
            serviceRecodeVosBeans.add(new maintainOrderRecordBean.ServiceRecordMapArrayBean());
        } else {
            for (pollingBlBean.ServiceRecodeVosBean serviceRecodeVo : serviceRecodeVos) {
    ...
    public void toNext() {
        ...
        List<pollingBlBean.ServiceRecodeVosBean> serviceRecodeVos = new ArrayList<>();
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            pollingBlBean.ServiceRecodeVosBean serviceRecodeVosBean1 = new pollingBlBean.ServiceRecodeVosBean();
        ...
        ((pollingBackTrackingActivity) getActivity()).blBean.setServiceRecodeVos(serviceRecodeVos);
        //跳转到下一界面
        Bundle bundle = new Bundle();
        bundle.putInt("currentPos", 1);
        start(bl_polling_3_fragment.newInstance(bundle));
    }
