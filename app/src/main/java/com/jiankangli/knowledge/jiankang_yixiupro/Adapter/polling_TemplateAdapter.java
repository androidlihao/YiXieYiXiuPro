...
import com.jiankangli.knowledge.jiankang_yixiupro.bean.pollingBlBean.WorkTemplateVosBean.TemplateTypeAndTemplateVosBean;
 ...
 * @description :模板适配器
 */
public class polling_TemplateAdapter extends BaseQuickAdapter<pollingBlBean.WorkTemplateVosBean, BaseViewHolder> {


    public polling_TemplateAdapter(@Nullable List<pollingBlBean.WorkTemplateVosBean> data) {
        super(data);
        //Step.1
        setMultiTypeDelegate(new MultiTypeDelegate<pollingBlBean.WorkTemplateVosBean>() {
            @Override
            protected int getItemType(pollingBlBean.WorkTemplateVosBean entity) {
                //根据你的实体类来判断布局类型
    ...
    @Override
    protected void convert(BaseViewHolder helper, final pollingBlBean.WorkTemplateVosBean item) {
        //得到当前操作项的位置
