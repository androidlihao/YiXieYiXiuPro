...
import com.jiankangli.knowledge.jiankang_yixiupro.Fragment.bl_polling.bl_polling_1_fragment;
import com.jiankangli.knowledge.jiankang_yixiupro.Fragment.bl_polling.bl_polling_2_fragment;
 ...
 * @description :巡检补录
 */
public class pollingBackTrackingActivity extends BaseActivity {
    @BindView(R.id.fl_content)
    FrameLayout flContent;
    public pollingBlBean blBean;
    ...
    private void initFragment() {
        if (findFragment(bl_upkeep_1_fragment.class) == null) {
            // 加载根Fragment
            loadRootFragment(R.id.fl_content, bl_polling_1_fragment.newInstance());
            //全局改变Fragment的动画
            setFragmentAnimator(new DefaultHorizontalAnimator());
        }
    }

    public void setBlBean(pollingBlBean blBean) {
        this.blBean = blBean;
    }
        ...
        if (getSupportFragmentManager().getBackStackEntryCount() > 1) {
            ...
            DialogUtil.showPositiveDialog(this, "警告", "关闭后，您填写的内容将会丢失", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ActivityCompat.finishAfterTransition(pollingBackTrackingActivity.this);
                }
            });
        }
    }
    ...
    public void onViewClicked() {
        if (getSupportFragmentManager().getBackStackEntryCount() == 2) {
            ((bl_polling_2_fragment) getSupportFragmentManager().getFragments().get(1)).toNext();
        }
    }
}
